...
WITH base_i AS (
  SELECT
    ...
    value, 
    _inserted_timestamp
  FROM {{ ref('silver___instructions2') }} i
...
WHERE
    i.block_id BETWEEN (
        SELECT
            LEAST(COALESCE(MAX(block_id), 105368)+1,153513616)
        FROM
            {{ this }}
        )
        AND (
        SELECT
            LEAST(COALESCE(MAX(block_id), 105368)+4000000,153513616)
        FROM
            {{ this }}
        ) 
{% elif is_incremental() %}
...
base_ii AS (
  SELECT
    ...
    _inserted_timestamp
  FROM {{ ref('silver___inner_instructions2') }} ii
