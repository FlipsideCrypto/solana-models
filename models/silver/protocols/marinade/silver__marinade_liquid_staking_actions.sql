...
{% if execute %}
    ...
    WHERE
        ...
        AND event_type IN ('deposit', 'depositStakeAccount', 'orderUnstake', 'claim','withdrawStakeAccount')
        AND succeeded
...
burns AS (
    ...
    INNER JOIN 
        (SELECT DISTINCT tx_id, block_timestamp::date as bt FROM base WHERE event_type IN ('orderUnstake','withdrawStakeAccount')) b 
        ON b.tx_id = a.tx_id 
        AND b.bt = a.block_timestamp::date
    WHERE
        a.succeeded
...
unstakes AS (
    SELECT
        ...
        b.burn_amount * pow(10, -9) AS msol_burned,
        a.program_id,
        a._inserted_timestamp
    FROM
        base a
    LEFT JOIN burns b 
        ...
        AND COALESCE(b.inner_index, 0) < COALESCE(a.next_liquid_staking_action_inner_index, 9999)
    WHERE
        a.event_type in ('orderUnstake','withdrawStakeAccount')
),
claims AS (
    SELECT
        a.block_id,
