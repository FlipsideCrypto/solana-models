...
{{ config(
    ...
    unique_key = ['lending_kamino_deposits_id'],
...
{% if execute %}
    {% set base_query %}
        CREATE OR REPLACE TEMPORARY TABLE silver.lending_kamino_deposits__tmp AS
        SELECT
            *
        FROM
    ...
    {% set between_stmts = fsc_utils.dynamic_range_predicate(
        "silver.lending_kamino_deposits__tmp",
...
decoded AS (
    SELECT
        ...
        CASE 
            ...
            THEN silver.udf_get_account_pubkey_by_name('depositAccounts > owner', decoded_instruction:accounts)
            ELSE silver.udf_get_account_pubkey_by_name('owner', decoded_instruction:accounts)
        END AS depositor,
        CASE 
            WHEN event_type = 'depositReserveLiquidityAndObligationCollateralV2' 
            THEN silver.udf_get_account_pubkey_by_name('depositAccounts > reserveCollateralMint', decoded_instruction:accounts)
...
SELECT 
    ...
    {{ dbt_utils.generate_surrogate_key(['a.tx_id', 'a.index', 'COALESCE(a.inner_index, -1)']) }} AS lending_kamino_deposits_id,
