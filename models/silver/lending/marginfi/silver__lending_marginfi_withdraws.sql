...
{{ config(
    ...
    unique_key = ['lending_marginfi_withdraws_id'],
...
{% if execute %}
    {% set base_query %}
        CREATE OR REPLACE TEMPORARY TABLE silver.lending_marginfi_withdraws__tmp AS
        SELECT
            *
        FROM
        ...
        WHERE
            ...
            AND event_type = 'lendingAccountWithdraw'
            AND succeeded
        {% if is_incremental() %}
    ...
    {% set between_stmts = fsc_utils.dynamic_range_predicate(
        "silver.lending_marginfi_withdraws__tmp",
...
SELECT 
    ...
    {{ dbt_utils.generate_surrogate_key(['a.tx_id', 'a.index', 'COALESCE(a.inner_index, -1)']) }} AS lending_marginfi_withdraws_id,
