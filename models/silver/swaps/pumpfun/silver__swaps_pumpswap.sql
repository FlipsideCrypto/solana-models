...
{% if execute %}
    {% set base_query %}
    CREATE OR REPLACE TEMPORARY TABLE silver.swaps_pumpswap__intermediate_tmp AS
    SELECT
        block_timestamp,
        block_id,
        tx_id,
    ...
    {% else %}
        AND _inserted_timestamp::DATE >= '2025-04-01'
    {% endif %}
    {% endset %}
    
    {% do run_query(base_query) %}
    {% set between_stmts = fsc_utils.dynamic_range_predicate(
        "silver.swaps_pumpswap__intermediate_tmp",
...
decoded AS (
    SELECT
        ...
        CASE 
            WHEN event_type = 'buy' THEN 
                silver.udf_get_account_pubkey_by_name('user_quote_token_account', decoded_instruction:accounts)
            ELSE 
                silver.udf_get_account_pubkey_by_name('user_base_token_account', decoded_instruction:accounts)
        ...
        CASE 
            WHEN event_type = 'buy' THEN 
                silver.udf_get_account_pubkey_by_name('pool_base_token_account', decoded_instruction:accounts)
            ELSE  
                silver.udf_get_account_pubkey_by_name('pool_quote_token_account', decoded_instruction:accounts)
...
SELECT
    ...
    swap_to_mint,
    _inserted_timestamp,
    {{ dbt_utils.generate_surrogate_key(['tx_id','index','inner_index']) }} AS swaps_pumpswap_id,
