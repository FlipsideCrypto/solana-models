...
SELECT
    ...
    log_index,
    program_id,
    {{ dbt_utils.generate_surrogate_key(['block_id','tx_id','index','inner_index','log_index','program_id']) }} as complete_decoded_logs_id,
    _inserted_timestamp
FROM
...
{% endif %}
qualify(ROW_NUMBER() over (PARTITION BY complete_decoded_logs_id
ORDER BY
