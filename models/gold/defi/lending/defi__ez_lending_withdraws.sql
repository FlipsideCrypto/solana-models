...
WITH withdraws AS (
    SELECT 
        ...
        amount_raw,
        amount,
        lending_kamino_withdraws_id AS ez_lending_withdraws_id
    FROM {{ ref('silver__lending_kamino_withdraws') }}
    {% if is_incremental() %}
    ...
    SELECT 
        ...
        amount_raw,
        amount,
        lending_marginfi_withdraws_id AS ez_lending_withdraws_id
    FROM {{ ref('silver__lending_marginfi_withdraws') }}
    {% if is_incremental() %}
    WHERE modified_timestamp >= '{{ max_modified_timestamp }}'
    {% endif %}
),
