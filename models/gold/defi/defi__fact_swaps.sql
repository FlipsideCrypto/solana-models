{{ config(
    ...
    post_hook = [enable_search_optimization('{{this.schema}}','{{this.identifier}}','ON EQUALITY(tx_id, swapper, swap_from_mint, swap_to_mint, program_id, fact_swaps_id)'),'ALTER TABLE {{this}} SET CHANGE_TRACKING = TRUE;'],
...
{% else %}
    AND modified_timestamp :: DATE < '{{ backfill_to_date }}'
{% endif %}
UNION ALL
SELECT
    block_timestamp,
