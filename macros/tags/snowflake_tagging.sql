  ...
  {% if execute %}
    ...
    {% for res in results -%}
        {% if res.node.meta.database_tags %}
            ...
            {% endfor %}
            {% for column in res.node.meta.database_tags.column %}
                {% for column_tag in res.node.meta.database_tags.column[column] %}
                    {{log(column_tag,info=True)}}
                    {{create_tag_if_missing(current_tags_in_schema,column_tag|upper)}}
                    {% set desired_tag_value = res.node.meta.database_tags.column[column][column_tag] %}
