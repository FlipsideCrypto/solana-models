{% macro generate_decoded_instructions_backfill_views(program_id) %}
    ...
    {% for i in range(min_block_id, max_block_id, step) %}
        ...
        {% set query = """create or replace view streamline.decoded_instructions_backfill_""" ~ program_id ~ """_""" ~ start_block ~ """_""" ~ end_block ~ """ AS 
...
{% macro queue_decoded_instructions_backfill() %}
    ...
    {% set results = run_query("""select
            table_schema,
            table_name
        from solana_dev.information_schema.views
        where table_name like 'DECODED_INSTRUCTIONS_BACKFILL_%';""").columns %}
    {% set schema_names = results[0].values() %}
    {% set table_names = results[1].values() %}
    {% for table_name in table_names %}
        {% set udf_call = if_data_call_function(
        func = schema_names[0] ~ ".udf_bulk_instructions_decoder(object_construct('sql_source', '" ~ table_name ~ "', 'external_table', 'decoded_instructions_2', 'sql_limit', '" ~ sql_limit ~ "', 'producer_batch_size', '" ~ producer_batch_size ~ "', 'worker_batch_size', '" ~ worker_batch_size ~ "', 'batch_call_limit', '" ~ batch_call_limit ~ "'))",
